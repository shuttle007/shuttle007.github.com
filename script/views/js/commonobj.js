define(["jquery"], function(require) {
	return commonObj = {
		...
		winH: $(window).height(),
		loadCanvas: function() {
			var $cvsDoms = $("#productul").find("canvas");
			if($cvsDoms.length > 0) {
				$cvsDoms.each(function() {
		...
		insertDiv: function(json) {
			...
			for(var i = 0; i < showlength; i++) {
				html += '<li><a href="#">' +
					...
					'<p class="ptitle pl10">日本Calbee卡乐比营养早餐麦片水果颗粒果仁谷物 原味800g</p>' +
					'<p class="pprice pl10">价格：<span class="green">￥59</span></p>' +
					'</div></a></li>';
			}
			$mainDiv.append(html);
		},
